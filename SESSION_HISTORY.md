...
---
## #38 (2026-05-03) — Phase 9 同期コア完成: 競合履歴 + Phase A 即時同期 + タグ・ToDo 同期

### 主要成果
- **Step 5e 競合履歴**: ConflictHistories テーブル追加 (schema 6→7)、双方向検出、200件上限、復元機能、データ保護から到達
- **Phase A 即時同期**: snapshotChanges() リアルタイム購読 + 編集 1.5秒 debounce アップロード、自端末発火フィルタ、authStateProvider 連動
- **タグ同期**: Tags.updatedAt 追加 (schema 7→8、ALTER TABLE 制約は customStatement で回避)、タグマスター + メモのタグ関連、Firestore に tagIds 配列で同期
- **ToDo 同期**: TodoLists doc に items + tagIds 埋め込み方式、UI 散在箇所すべてで scheduleUploadTodoList、リアルタイム購読 + debounce
- **競合履歴 UI 統一**: 端末非依存表示、CupertinoIcons.delete_simple、200件上限明記

### 修正したバグ
- 同名タグ複数で getSingleOrNull が例外 → get() で1件目採用に変更
- SQLite ALTER TABLE で NOT NULL + 非const デフォルト不可 → customStatement で対応
- リアルタイム受信トーストが煩い → 撤去（即時反映で十分）

### 検証
- iPhone SE 3 / iPhone 13 mini シミュで実競合・実時同期テスト済み
- タグ追加 → 1.5秒以内に他端末反映 ✓
- ToDo 追加・編集・並び替え・チェック → 即時反映 ✓
- 競合発生 → 両端末の競合履歴に記録 ✓

### 既知の問題
- 両端末で独立 seed → 同名タグ・メモが別 id で重複（運用上の注意）
- リリース時は seed なしなので非問題

### 次セッション (#39) 候補
- 画像同期（Firebase Storage 経由）
- 削除の同期（tombstone 方式推奨）
- サブスク（RevenueCat / in_app_purchase）

---
## #39 (2026-05-04)

### Phase 9 画像同期 完了
- Firebase Blaze 化（Memolette 専用 Cloud 請求先アカウント新規作成、予算アラート ¥750/月）
- Firebase Storage 有効化（バケット作成、テストモード、US-EAST1）
- pubspec.yaml に firebase_storage: ^13.0.0 追加
- MemoImages テーブルに remoteUrl カラム追加（schemaVersion 8→9）
- メモ doc に images: [{id, url, sortOrder}] 配列を埋込
- Storage パス: users/{uid}/memo_images/{imageId}.jpg
- SyncService に画像系メソッド一式（_uploadOneImage / _ensureMemoImagesUploaded / _syncImagesFromRemote / _imageToMap / _extractImages / _RemoteImageMeta）
- uploadOneMemo / uploadAllMemos / downloadAllMemos / リアルタイム購読 すべてに画像処理を統合

### 編集中メモへの同期反映
- memo_input_area に watchMemoById 購読追加 → リモート content / title 変化を _applyMemoData で反映
- block_editor に watchMemoImages 購読追加 → 画像 DL 後追いを再構築でカバー
- 再構築時は db.getMemoById で最新 content を使う（_serialize() だと古いマーカーで末尾追加問題）
- _initialized / _suppressImagesWatch ガードで初期化前・自分の操作中の誤検知を防止
- database に watchMemoById / setMemoImageRemoteUrl / upsertMemoImageFromRemote / removeMemoImageLocalOnly を追加

### block_editor の細かい UX 改善
- 画像挿入時の改行吸収（空行頭で挿入時に視覚的な空行ダブりを防ぐ）
- ×ボタン編集モード時のみ表示（!widget.readOnly）
- ×ボタンのヒット領域を 36×40px 相当に拡大、上方向は 4px に控える（直上テキストと干渉しない）
- 画像削除後にカーソル復帰（編集モード継続、キーボード閉じない）

### バグ修正
- ビルドキャッシュが古い状態で起動 → 修正済み seedDummyBulkMemos がクラッシュ → build/ と .dart_tool/build を削除して再ビルドで解決
- watchMemoImages の初回 listener が _initAsync 完了前に発火 → initialContent のテキストが消える問題

### 動作確認済み
- 両シミュ (iPhone SE 3 / iPhone 13 mini) で双方向画像同期（追加 / 位置正しい / 編集中メモへの反映）

### 残タスク（次セッション #40）
- 画像削除の同期テスト
- メモ削除の同期（tombstone 方式）
- Step 4 サブスク（プラン設計済み）
- Firestore セキュリティルール本番化（30日以内）
- データ保護画面のダイアログ文言検討 / 復元ワンクッション化
